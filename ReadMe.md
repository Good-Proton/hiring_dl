...
* `slice(start?: number, end?: number): Promise<PersonId[]>` - асинхронный метод получения части коллекции в виде массива 
...
1. При разработке решения, надо учитывать, что размер списка `target` может доходить до 50000 профилей, в рамках одной 
